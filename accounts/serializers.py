...
class CourierSerializer(serializers.ModelSerializer):
    ...
    covered_cities_names = serializers.SerializerMethodField()
    region = serializers.SerializerMethodField()  # Add a region field

    class Meta:
        model = Courier
        fields = '__all__'  # or list fields explicitly, including 'region' and 'covered_cities_names'

    def get_covered_cities_names(self, obj):
        cities = obj.covered_cities.all()
        return ", ".join(city.name for city in cities)

    def get_region(self, obj):
        # Return the region of the first city in covered_cities, if it exists
        first_city = obj.covered_cities.first()
        if first_city and first_city.region:
            return {
                "id": first_city.region.id,
                "name": first_city.region.name
            }
        return None
