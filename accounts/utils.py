...
def import_products_from_excel(excel_file, send_sms=True):
    """
    ...
    For each newly created product, a 10-digit secret key is generated and stored.
    An SMS is then sent to the phone number with a link that includes the secret key:
      https://t.me/tujjoruz_bot?tj=secret_key

    Returns a list of messages describing the result.
    """
    """
    Import products from an Excel file using openpyxl, with the option
    to send or skip SMS messages (send_sms flag).
    """
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        try:
            ...
            if created:
                imported_count += 1
                # --- Only send SMS if send_sms == True ---
                if send_sms:
                    from .sms import transmit_sms
                    sms_text = (
                        f"{order_number} raqamli buyurtmangizni kuryer manzilingizga yetkazib "
                        f"berishi uchun joylashuv manzilingizni quyidagi botga yuboring: "
                        f"https://t.me/tujjoruz_bot?tj={secret_key}"
                    )
                    sms_id = order_number
                    sms_response = transmit_sms(sms_text, sms_id, phone)
                    if not sms_response.get("status"):
                        messages.append(f"SMS failed for order {order_number}: {sms_response.get('response')}")
            else:
